...
void tmr_wait_ms(int timer, int ms){
    if (timer == TIMER1) {
        ...
        int period = (FCY / 256) * ms / 1000; // Calculate period value
...
void tmr_setup_period(int timer, int ms){
    ...
    if (timer == TIMER1) {
        ...
        T1CONbits.TON = 0; // Stop timer
        TMR1 = 0; // Clear timer register
        T1CONbits.TCKPS = 0b10; // Set prescaler to 64
        long long period = (FCY / 64) * ms / 1000; // Calculate period value
        PR1 = period; // Set period register
        T1CONbits.TON = 1; // Start timer
        ...
        long long period = ((long long)FCY / 256) * ms / 1000; // 140625 for 500ms
        PR5 = (period >> 16) & 0xFFFF;
        PR4 = period & 0xFFFF;
        IFS1bits.T4IF = 0;  // clear flag
        IEC1bits.T4IE = 1;  // enable Timer4 interrupt
        IPC6bits.T4IP = 1;  // priority level 1
        T4CONbits.TON = 1;
